...
namespace PrintStreamer.Endpoints.Api.Timelapses
{
    ...
    {
        private readonly ILogger<TimelapseGenerateEndpoint> _logger;
        private readonly TimelapseManager _timelapseManager;

        public TimelapseGenerateEndpoint(ILogger<TimelapseGenerateEndpoint> logger, TimelapseManager timelapseManager)
        {
            _logger = logger;
        ...
        public override async Task HandleAsync(TimelapseNameRequest req, CancellationToken ct)
        {
            try
            {
                ...
                _logger.LogInformation("Generating video from {FrameCount} frames: {VideoPath}", frameFiles.Length, videoPath);
                ...
                {
                    _logger.LogInformation("Video created successfully: {VideoPath}", videoPath);
                    HttpContext.Response.StatusCode = 200;
                    await HttpContext.Response.WriteAsJsonAsync(new { success = true, videoPath }, ct);
                }
                else
                {
                    _logger.LogError("ffmpeg failed with exit code {ExitCode}", proc.ExitCode);
                    _logger.LogError("ffmpeg output: {Output}", output);
                    HttpContext.Response.StatusCode = 500;
            ...
            {
                _logger.LogError(ex, "Error generating video: {Message}", ex.Message);
                HttpContext.Response.StatusCode = 500;
